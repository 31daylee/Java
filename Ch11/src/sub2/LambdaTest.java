 ...
 * 람다식
 * - 함수형 프로그래밍을 지원하는 구문
 * - 인터페이스 타입을 @FunctionalInterface 선언해시 람다식 정의
 */
...
public class LambdaTest {
	...
	public static void main(String[] args) {
		...
		A a2 = (a, b) -> {return a + b;}; //인터페이스 A와 동일한 매개변수 입력
		A a3 = (a, b) -> a+b; //중괄호 생략 시 리턴타입 생략 
		...
		B b3 = System.out::println; //인터페이스의 추상 메서드를 출력하게 되면 println 이 되구나		
		...
		C c1 = ()->{return true;};// 불린 타입이므로 리턴은 true or false
		C c2 = () -> false;
		...
		// 람다식 활용
		Runnable run = () -> { //인터페이스 Runnable 의 run 메서드로 구현 
			for(int i=1; i<=10; i++) {
